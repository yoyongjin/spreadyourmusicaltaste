...
const loadNextPosts = () => {
  ...
  if (window.pageYOffset + window.innerHeight
    ...
    setTimeout(async () => { // 서버와 통신하여 다음 페이지 요청
      ...
      const res = await request.get(`/posts?_page=${++currPageNum}&_limit=6&_sort=${sortBy},id&_order=desc,desc`);
...
window.onload = () => {
  (async () => {
    ...
    const res = await request.get(`/posts?_page=1&_limit=6&_sort=${sortBy},id&_order=desc,desc`); // 첫번째 페이지 자료 요청
